...
def test_chat_schema_has_required_fields():
    ...
    with (
        patch("publish.boto3.resource") as mock_resource,
        patch("publish.boto3.client") as mock_client,
    ):
        # Mock CloudFormation describe_stacks
        mock_cfn = MagicMock()
        mock_cfn.describe_stacks.return_value = {
            "Stacks": [
                {"Outputs": [{"OutputKey": "ConfigurationTableName", "OutputValue": "test-table"}]}
            ]
        }
...
def test_chat_default_values():
    ...
    with (
        ...
        assert default_item["chat_primary_model"] == "anthropic.claude-haiku-4-5-20251001-v1:0"
        assert default_item["chat_fallback_model"] == "amazon.nova-micro-v1:0"
