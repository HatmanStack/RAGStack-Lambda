...
def mock_bedrock_response():
    ...
    return {
        'output': {
            'text': 'Based on the documents, the main topics are cloud architecture and AWS services.'
        },
        'citations': [
            {
                'retrievedReferences': [
...
@patch.dict('os.environ', {'KNOWLEDGE_BASE_ID': 'test-kb-123', 'AWS_REGION': 'us-east-1', 'CONFIGURATION_TABLE_NAME': 'test-config-table'})
@patch('index.config_manager')
@patch('index.boto3.client')
def test_lambda_handler_success(mock_boto3_client, mock_config_manager, valid_event, lambda_context,
...
@patch.dict('os.environ', {'KNOWLEDGE_BASE_ID': 'test-kb-123', 'CONFIGURATION_TABLE_NAME': 'test-config-table'})
@patch('index.config_manager')
@patch('index.boto3.client')
def test_lambda_handler_empty_query(mock_boto3_client, mock_config_manager, lambda_context):
    ...
    # Verify boto3.client was still called to create bedrock_agent (but retrieve_and_generate not called)
...
@patch('index.config_manager')
@patch('index.boto3.client')
def test_lambda_handler_uses_runtime_config(mock_boto3_client, mock_config_manager,
...
@patch('index.config_manager')
@patch('index.boto3.client')
def test_lambda_handler_uses_correct_region_in_model_arn(mock_boto3_client, mock_config_manager,
                                                           valid_event, lambda_context):
