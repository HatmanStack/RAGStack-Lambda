...
def test_skip_ui_optional():
    ...
    # Without --skip-ui
    args = parser.parse_args(['--project-name', 'test', '--admin-email', 'test@example.com', '--region', 'us-east-1'])
    assert args.skip_ui is False

    # With --skip-ui
    args = parser.parse_args(['--project-name', 'test', '--admin-email', 'test@example.com', '--region', 'us-east-1', '--skip-ui'])
